...
impl Whoop {
    ...
    async fn handle_notification(
        ...
        // Tell the compiler explicitly that the entire match returns `anyhow::Result<bool>`
        let result: anyhow::Result<bool> = match packet.uuid {
            DATA_FROM_STRAP => {
                ...
                match data {
                    ...
                    WhoopData::HistoryMetadata { data, cmd, .. } => match cmd {
                        MetadataType::HistoryComplete => Ok(true),
                        MetadataType::HistoryStart => Ok(false),
                    ...
                    WhoopData::RunAlarm { .. } => Ok(false),
                    WhoopData::Event { .. } => Ok(false),
                    WhoopData::UnknownEvent { .. } => Ok(false),
                }
            }
            _ => {
                Ok(false)
        ...
        // Now `result` is definitely `anyhow::Result<bool>`.
        result?;
